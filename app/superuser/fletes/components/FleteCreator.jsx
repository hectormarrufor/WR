...
export default function FleteCreator() {
  ...
  const { data: estimacion, isLoading: calcLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const activo = activos.find((a) => a.id === formValues.activoPrincipalId);
      // Si tienes API para params operativos, fetch aquí; sino hardcode temporal
      const paramsOperativos = {
        gastosFijosMensuales: 8000, // ajusta con tus números reales
        viajesMensualesPromedio: 18,
        kmMensualesPromedio: 3500,
        viaticosPorDia: 25,
      };

      const horasEstimadas = (rutaData.distanciaTotal / 50) + 4; // avg 50 km/h + 4h operativas

      return calcularCostoFlete({
        distanciaKm: rutaData.distanciaTotal,
        tonelaje: formValues.tonelaje,
        tipoCarga: formValues.tipoCarga,
        activoPrincipal: activo,
        horasEstimadas,
        paramsOperativos,
