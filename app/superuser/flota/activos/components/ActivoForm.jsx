...
import {
    ...
    Accordion, Box, Badge, Alert
} from '@mantine/core';
import { useForm } from '@mantine/form';
import { IconCheck, IconCoin, IconTool, IconCalculator, IconInfoCircle, IconSettings } from '@tabler/icons-react';
import { notifications } from '@mantine/notifications';
import { useAuth } from '@/hooks/useAuth';
import ImageDropzone from '../components/ImageDropzone';
import ComponenteInstaller from './ComponenteInstaller';
import ModalConfigurarTanque from '../[id]/components/ModalConfigurarTanque';
...
export default function ActivoForm({ matricesData = [], plantilla, tipoActivo, onCancel, initialData = null, matricesCostos = [] }) {
    ...
    // 🔥 ESTADOS PARA EL MODO LOCAL 🔥
    const [modalTanqueOpened, setModalTanqueOpened] = useState(false);
    const tieneTanquePlantilla = !!plantilla?.configuracionTanque;
    ...
    const form = useForm({
        initialValues: {
            ...
            tipoTanque: 'original',
            // 🔥 ALMACENAMIENTO BORRADOR 🔥
            capacidadTanqueLocal: null,
            configuracionTanqueLocal: null,
        ...
        validate: {
            ...
            tipoTanque: (val) => (!val ? 'Seleccione una opción' : null)
        }
    });
    ...
    // ... (Mantén aquí los useEffects de fetchConfig y fetchInventario, y los cálculos de overhead que ya tienes) ...
    useEffect(() => {
    ...
    const handleNextStep = () => {
        let hasError = false;
        if (active === 0) {
            const errs = ['codigoInterno', 'tipoTanque'].map(f => form.validateField(f).hasError);
                        ...
                        message: (
                            <div>
                                ...
                                <Group justify="space-between" mb={4}>
                                    ...
                                    <Text size="sm" fw={700}>{horasAnteriores} <Text span c="dimmed">➡️</Text> {horasNuevas} hrs</Text>
                                </Group>
                                <Group justify="space-between">
                                    <Text size="xs" c="dimmed" fw={600}>Nuevo Overhead:</Text>
                                    <Text size="sm" fw={800} c={colorAlerta}>${overheadAnterior.toFixed(2)} <Text span c="dimmed">➡️</Text> ${overheadNuevo.toFixed(2)}/hr</Text>
                                </Group>
                            </div>
                        ),
                        color: 'violet',
    ...
    return (
        <Stack gap="xl">
            ...
            <Stepper active={active} onStepClick={setActive} allowNextStepsSelect={isEditing}>
                <Stepper.Step label="Físico" description="Identificación">
                    <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="xl" mt="md">
                        <Stack>
                            ...
                            {/* 🔥 LOGICA INTELIGENTE DE TANQUES 🔥 */}
                            <Paper withBorder p="sm" bg="gray.0" radius="md">
                                <Select 
                                    label="Configuración de Tanque" 
                                    required
                                    disabled={isEditing}
                                    data={[
                                        { value: 'original', label: 'Original de Fábrica' },
                                        { value: 'aftermarket', label: 'Modificado / Aftermarket' },
                                        { value: 'none', label: 'No aplica / No tiene tanque' }
                                    ]} 
                                    {...form.getInputProps('tipoTanque')} 
                                />

                                {!isEditing && form.values.tipoTanque !== 'none' && (
                                    <Stack mt="sm" gap="xs">
                                        {form.values.tipoTanque === 'original' && tieneTanquePlantilla && !form.values.capacidadTanqueLocal && (
                                            <Alert color="teal" variant="light" p="xs" icon={<IconInfoCircle size={16} />}>
                                                Heredará la geometría de <b>{parseFloat(plantilla.capacidadTanque).toFixed(2)}L</b> de la plantilla.
                                            </Alert>
                                        )}
                                        
                                        {form.values.tipoTanque === 'original' && !tieneTanquePlantilla && !form.values.capacidadTanqueLocal && (
                                            <Alert color="orange" variant="light" p="xs" icon={<IconAlertCircle size={16} />}>
                                                La plantilla aún no tiene geometría. Configúrala aquí para heredarla al resto.
                                            </Alert>
                                        )}

                                        {form.values.capacidadTanqueLocal && (
                                            <Alert color="blue" variant="filled" p="xs" icon={<IconCheck size={16} />}>
                                                Borrador listo: {form.values.capacidadTanqueLocal}L calculados.
                                            </Alert>
                                        )}

                                        {(!tieneTanquePlantilla || form.values.tipoTanque === 'aftermarket') && (
                                            <Button 
                                                variant="light" 
                                                color="blue" 
                                                size="xs" 
                                                leftSection={<IconSettings size={14} />}
                                                onClick={() => setModalTanqueOpened(true)}
                                            >
                                                {form.values.capacidadTanqueLocal ? 'Modificar Borrador' : 'Configurar Geometría Ahora'}
                                            </Button>
                                        )}
                                    </Stack>
                                )}
                            ...
                            <Divider label="Pesos y Capacidades" my="sm" />
                            <NumberInput
                        ...
                        <Stack>
                            ...
                            <Divider label="Legal e Instrumentos" my="sm" />
                ...
                {/* ... (Mantén aquí el Stepper.Step Financiero y de Componentes tal cual los pasaste, no los toco para no alargar) ... */}
            ...
            {/* 🔥 EL MODAL EN MODO LOCAL 🔥 */}
            <ModalConfigurarTanque 
                opened={modalTanqueOpened}
                onClose={() => setModalTanqueOpened(false)}
                modoLocal={true}
                activo={{}} 
                onSuccess={(dataLocal) => {
                    form.setFieldValue('configuracionTanqueLocal', dataLocal.configuracionTanque);
                    form.setFieldValue('capacidadTanqueLocal', dataLocal.capacidadNeta);
                }}
            />
        </Stack>
    );
