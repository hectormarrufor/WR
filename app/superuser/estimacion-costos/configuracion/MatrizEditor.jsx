...
import { IconTrash, IconPlus, IconCalculator, IconDeviceFloppy, IconInfoCircle, IconArrowRight } from '@tabler/icons-react';
...
export default function MatrizEditor({ matrizId }) {
    ...
    const [modalImpactoOpened, setModalImpactoOpened] = useState(false);
    const [activosImpactados, setActivosImpactados] = useState([]);
        ...
        Promise.all([
            ...
            .then(([matrizData, configData]) => {
                ...
                if (configData && configData.horasTotalesFlota) {
                    // Evitamos dividir por cero por si no hay ninguna unidad activa
                    const unidadesActivas = configData.cantidadUnidadesActivas || 1; 
                    
                    setHorasAnuales(configData.horasTotalesFlota / unidadesActivas);
                    setHorasPorMes((configData.horasTotalesFlota / unidadesActivas) / 12);
                }
            })
            .catch(err => {
    ...
    const guardarCambios = async () => {
        ...
        try {
            ...
            const data = await res.json(); // Leemos la respuesta JSON real

            if (res.ok) {
                // Si la API nos devuelve activos afectados, mostramos el modal
                if (data.reporteImpacto && data.reporteImpacto.length > 0) {
                    setActivosImpactados(data.reporteImpacto);
                    setModalImpactoOpened(true);
                } else {
                    notifications.show({ title: 'Guardado', message: 'Estructura actualizada sin impacto a terceros.', color: 'green' });
                }
            } else {
                throw new Error(data.error || 'El servidor rechazó los datos');
            }
        } catch (e) {
            notifications.show({ title: 'Error', message: e.message || 'No se pudo guardar la estructura', color: 'red' });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <>
            ...
            <Modal
                opened={modalImpactoOpened}
                onClose={() => setModalImpactoOpened(false)}
                title={<Text fw={700} size="lg" c="blue.9">Impacto en Activos Asociados</Text>}
                size="xl"
                centered
                closeOnClickOutside={false} // Evitamos que lo cierre sin querer
            >
                <Text mb="md" size="sm" c="dimmed">
                    Al modificar esta Estructura de Costos, los siguientes activos han actualizado sus tarifas automáticamente:
                </Text>
                    ...
                    <Table striped highlightOnHover>
                        <Table.Thead bg="gray.1">
                            <Table.Tr>
                                ...
                                <Table.Th>Var. Mantenimiento ($/Km)</Table.Th>
                                <Table.Th>Fijo Posesión ($/Hora)</Table.Th>
                            </Table.Tr>
                        </Table.Thead>
                        <Table.Tbody>
                            {activosImpactados.map((activo, i) => (
                                <Table.Tr key={i}>
                                    <Table.Td>
                                        <Text fw={700}>{activo.codigoInterno}</Text>
                                        <Text size="xs" c="dimmed">{activo.tipo}</Text>
                                    </Table.Td>
                                    <Table.Td>
                                        <Group gap="xs" align="center" wrap="nowrap">
                                            <Text size="sm" c="gray.6">${parseFloat(activo.oldKm).toFixed(3)}</Text>
                                            <IconArrowRight size={14} color="gray" />
                                            <Badge color={activo.newKm > activo.oldKm ? 'red' : (activo.newKm < activo.oldKm ? 'green' : 'gray')} variant="light">
                                                ${parseFloat(activo.newKm).toFixed(3)}
                                            </Badge>
                                        </Group>
                                    </Table.Td>
                                    <Table.Td>
                                        <Group gap="xs" align="center" wrap="nowrap">
                                            <Text size="sm" c="gray.6">${parseFloat(activo.oldHora).toFixed(2)}</Text>
                                            <IconArrowRight size={14} color="gray" />
                                            <Badge color={activo.newHora > activo.oldHora ? 'red' : (activo.newHora < activo.oldHora ? 'green' : 'gray')} variant="light">
                                                ${parseFloat(activo.newHora).toFixed(2)}
                                            </Badge>
                                        </Group>
                                    </Table.Td>
                                </Table.Tr>
                ...
                <Group justify="flex-end" mt="xl">
                    <Button onClick={() => setModalImpactoOpened(false)} color="blue">
                        Entendido
                    </Button>
                </Group>
            </Modal>
