// app/api/superuser/proveedores/[id]/route.js
...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    if (!body.nombre || !body.rif || !body.telefono) {
      return NextResponse.json({ message: 'Nombre, RIF y teléfono son campos requeridos para la actualización.' }, { status: 400 });
    }
    ...
    if (error.name === 'SequelizeUniqueConstraintError') {
      return NextResponse.json({ message: 'Ya existe otro proveedor con ese RIF.', error: error.message }, { status: 409 });
    }
