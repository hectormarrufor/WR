...
export async function GET(request) {
  try {
    const cuentas = await db.CuentaTerceros.findAll({
...
export async function POST(request) {
  try {
    ...
    const { tipoEntidad, entidadId, ...rest } = body;
    tipoEntidad === 'proveedor' ? rest.proveedorId = entidadId : rest.empleadoId = entidadId;
    const nuevaCuenta = await db.CuentaTerceros.create(rest);
